...
const Sidebar = (props) => {
  const {notes, setNotes, activeNote, setActiveNote} = props;
  const onAddNote = () => {
    const newNote = {
      id: uuid(),
      title: "新しいノート",
  ...
  const sortedNotes = notes.sorts((a, b) => b.modDate - a.modDate)
  return (
    <div className='side-bar'>
      ...
      <div className='sidebar-content'>
        {notes.map((note) => {
          return (
            <div 
            className={`content-note ${note.id === activeNote && "active"}`} 
            key={note.id} 
